...
public class RadioNameHookEntry implements IXposedHookLoadPackage {
    ...
    public void handleLoadPackage(XC_LoadPackage.LoadPackageParam lpparam) {
        ...
        if (isWidget) {
            ...
            // v18: Canvas.drawText хук для диагностики — ищем где рисуется "FM 100.4"
        ...
        if (isMediaCenter) {
            ...
            // v23→v24: Только getTitle, БЕЗ getRadioFrequency (ломает шкалу)
        ...
        if (isLauncher) {
            ...
            // v26: Хукаем updateAppWidget в launcher — перехватываем RemoteViews
            hookLauncherUpdateAppWidget(lpparam);
            // v26: Хукаем RadioRulerView.setRadioFreq — кастомный View рисующий шкалу
            hookLauncherRadioRulerView(lpparam);
            // v19: ПОЛНЫЙ СКАНЕР
            deepScanProcess(lpparam);
        }

        // v19: Сканер и для виджет-процесса
        if (isWidget) {
            deepScanProcess(lpparam);
        }
    ...
    /**
     * v24: Хукаем только getTitle() в DIM API — безопасно, не ломает шкалу
     */
    private void hookDimApiTitle(XC_LoadPackage.LoadPackageParam lpparam) {
        ...
        for (String cn : candidates) {
            try {
                ...
                try {
                    XposedHelpers.findAndHookMethod(cls, "getTitle",
                        new XC_MethodHook() {
                            @Override
                            protected void afterHookedMethod(MethodHookParam param) {
                                try {
                                    ...
                                    XposedBridge.log(TAG + ": >>> DIM.getTitle() '' -> '" + found + "' (freq=" + freq + ")");
                                } catch (Throwable t) { }
                            }
                        });
                    XposedBridge.log(TAG + ": ✓ DIM " + cn + ".getTitle");
                } catch (Throwable t) { }
    ...
    /**
     * v18: Хукаем Canvas.drawText — диагностика + подмена текста виджета.
     */
    private void hookCanvasDrawText(XC_LoadPackage.LoadPackageParam lpparam) {
        // Overload 1: drawText(String, float, float, Paint)
        try {
            XposedHelpers.findAndHookMethod(android.graphics.Canvas.class, "drawText",
                ...
                new XC_MethodHook() {
                    ...
                    protected void beforeHookedMethod(MethodHookParam param) {
                        try {
                            ...
                            if (text == null || text.length() < 2 || text.length() > 20) return;
                            String freq = extractFreq(text);
                            if (freq == null) return;
                            String found = StationsDatabase.findName(freq);
                            if (found == null) return;
                            XposedBridge.log(TAG + ": >>> DRAWTEXT1 '" + text + "' -> '" + found + "'");
                            param.args[0] = found;
                        } catch (Throwable t) { /* skip */ }
                    }
                });
            XposedBridge.log(TAG + ": ✓ hooked Canvas.drawText(String,float,float,Paint)");
        } catch (Throwable t) { }

        // Overload 2: drawText(char[], int, int, float, float, Paint)
        try {
            XposedHelpers.findAndHookMethod(android.graphics.Canvas.class, "drawText",
                ...
                new XC_MethodHook() {
                    ...
                    protected void beforeHookedMethod(MethodHookParam param) {
                        try {
                            ...
                            String found = StationsDatabase.findName(freq);
                            if (found == null) return;
                            XposedBridge.log(TAG + ": >>> DRAWTEXT2 '" + text + "' -> '" + found + "'");
                            char[] nc = found.toCharArray();
                            param.args[0] = nc; param.args[1] = 0; param.args[2] = nc.length;
                        } catch (Throwable t) { /* skip */ }
                    }
                });
            XposedBridge.log(TAG + ": ✓ hooked Canvas.drawText(char[],int,int,float,float,Paint)");
        } catch (Throwable t) { }

        // Overload 3: drawText(String, int start, int end, float, float, Paint) — КЛЮЧЕВОЙ!
        try {
            XposedHelpers.findAndHookMethod(android.graphics.Canvas.class, "drawText",
                String.class, int.class, int.class, float.class, float.class, android.graphics.Paint.class,
                new XC_MethodHook() {
                    @Override
                    protected void beforeHookedMethod(MethodHookParam param) {
                        try {
                            String text = (String) param.args[0];
                            int start = (Integer) param.args[1];
                            int end = (Integer) param.args[2];
                            if (text == null || end - start < 2 || end - start > 20) return;
                            String sub = text.substring(start, end);
                            String freq = extractFreq(sub);
                            if (freq == null) return;
                            String found = StationsDatabase.findName(freq);
                            if (found == null) return;
                            XposedBridge.log(TAG + ": >>> DRAWTEXT3 '" + sub + "' -> '" + found + "'");
                            param.args[0] = found;
                            param.args[1] = 0;
                            param.args[2] = found.length();
                        } catch (Throwable t) { /* skip */ }
                    }
                });
            XposedBridge.log(TAG + ": ✓ hooked Canvas.drawText(String,int,int,float,float,Paint)");
        } catch (Throwable t) { }

        // Overload 4: drawText(CharSequence, int, int, float, float, Paint)
        try {
            XposedHelpers.findAndHookMethod(android.graphics.Canvas.class, "drawText",
                ...
                new XC_MethodHook() {
                    ...
                    protected void beforeHookedMethod(MethodHookParam param) {
                        try {
                            ...
                            int start = (Integer) param.args[1];
                            int end = (Integer) param.args[2];
                            if (cs == null || end - start < 2 || end - start > 20) return;
                            String sub = cs.subSequence(start, end).toString();
                            String freq = extractFreq(sub);
                            if (freq == null) return;
                            String found = StationsDatabase.findName(freq);
                            if (found == null) return;
                            XposedBridge.log(TAG + ": >>> DRAWTEXT4 '" + sub + "' -> '" + found + "'");
                            param.args[0] = found;
                            param.args[1] = 0;
                            param.args[2] = found.length();
                        } catch (Throwable t) { /* skip */ }
            ...
            XposedBridge.log(TAG + ": ✓ hooked Canvas.drawText(CharSequence,int,int,float,float,Paint)");
        } catch (Throwable t) { }

        // Также ищем RadioRulerView в launcher и хукаем setRadioFreq / onDraw
        try {
            Class<?> rrv = lpparam.classLoader.loadClass("ecarx.launcher3.widget.radio.RadioRulerView");
            XposedBridge.log(TAG + ": ✓ found RadioRulerView in " + lpparam.packageName);
            // Дампим все методы
            for (Method m : rrv.getDeclaredMethods()) {
                Class<?>[] p = m.getParameterTypes();
                StringBuilder sb = new StringBuilder();
                for (Class<?> c : p) sb.append(c.getSimpleName()).append(",");
                XposedBridge.log(TAG + ":   RRV METHOD: " + m.getName() + "(" + sb + ")");
            }
            // Хукаем setRadioFreq если есть
            try {
                for (Method m : rrv.getDeclaredMethods()) {
                    if (m.getName().equals("setRadioFreq") || m.getName().equals("setTypeAndFreq")
                            || m.getName().equals("setFreq") || m.getName().equals("setPointerValue")) {
                        final String mn = m.getName();
                        XposedBridge.hookMethod(m, new XC_MethodHook() {
                            @Override
                            protected void beforeHookedMethod(MethodHookParam param) {
                                StringBuilder args = new StringBuilder();
                                for (Object a : param.args) args.append(a).append(",");
                                XposedBridge.log(TAG + ": >>> RRV." + mn + "(" + args + ")");
                            }
                        });
                        XposedBridge.log(TAG + ": ✓ hooked RRV." + mn);
                    }
                }
            } catch (Throwable t) { }
        } catch (ClassNotFoundException e) {
            ...
            String[] candidates = {
                "ecarx.launcher3.widget.RadioRulerView",
                "ecarx.launcher3.view.RadioRulerView",
                "ecarx.launcher3.radio.RadioRulerView",
                "ecarx.launcher3.widget.media.RadioRulerView",
            };
            for (String cn : candidates) {
                try {
                    Class<?> rrv = lpparam.classLoader.loadClass(cn);
                    XposedBridge.log(TAG + ": ✓ found " + cn);
                    for (Method m : rrv.getDeclaredMethods()) {
                        XposedBridge.log(TAG + ":   RRV METHOD: " + m.getName() + "(" + m.getParameterCount() + ")");
                    }
                    break;
                } catch (ClassNotFoundException e2) { /* next */ }
            }
            XposedBridge.log(TAG + ": RadioRulerView not found in " + lpparam.packageName);
        }
    }

    /**
     * v19: ПОЛНЫЙ СКАНЕР ПРОЦЕССА.
     * Ищет ВСЕ классы содержащие ключевые слова (radio, freq, ruler, widget, media, fold)
     * и дампит их методы. Также хукает TextView.setText в процессе.
     */
    private void deepScanProcess(XC_LoadPackage.LoadPackageParam lpparam) {
        XposedBridge.log(TAG + ": ===== DEEP SCAN " + lpparam.packageName + " =====");

        // 1. Сканируем все загруженные классы через DexFile
        try {
            String apkPath = lpparam.appInfo.sourceDir;
            XposedBridge.log(TAG + ": SCAN APK: " + apkPath);

            dalvik.system.DexFile dexFile = new dalvik.system.DexFile(apkPath);
            java.util.Enumeration<String> entries = dexFile.entries();

            String[] keywords = {"radio", "ruler", "freq", "station", "fold", "widget",
                                  "playback", "tuner", "channel", "pointer", "dial"};
            ...
            while (entries.hasMoreElements()) {
                ...
                // Пропускаем android.*, java.*, kotlin.*, androidx.*
                if (className.startsWith("android.") || className.startsWith("java.")
                    || className.startsWith("kotlin.") || className.startsWith("androidx.")
                    || className.startsWith("com.google.") || className.startsWith("org.greenrobot.")) continue;
                ...
                try {
                    ...
                    found++;
                    XposedBridge.log(TAG + ": SCAN CLASS: " + className + " (" + methods.length + " methods)");

                    for (Method m : methods) {
                        String mn = m.getName().toLowerCase();
                        // Логируем только интересные методы
                        if (mn.contains("radio") || mn.contains("freq") || mn.contains("title")
                            || mn.contains("text") || mn.contains("draw") || mn.contains("update")
                            || mn.contains("refresh") || mn.contains("set") || mn.contains("pointer")
                            || mn.contains("ruler") || mn.contains("station") || mn.contains("name")
                            || mn.contains("content") || mn.contains("info") || mn.contains("display")) {

                            Class<?>[] params = m.getParameterTypes();
                            StringBuilder sb = new StringBuilder();
                            for (Class<?> p : params) sb.append(p.getSimpleName()).append(",");
                            XposedBridge.log(TAG + ":   -> " + m.getName()
                                + "(" + sb + ") ret=" + m.getReturnType().getSimpleName());
                        }
                    }

                    // Дампим поля с интересными именами
                    for (java.lang.reflect.Field f : cls.getDeclaredFields()) {
                        String fn = f.getName().toLowerCase();
                        if (fn.contains("freq") || fn.contains("radio") || fn.contains("station")
                            || fn.contains("title") || fn.contains("name") || fn.contains("pointer")
                            || fn.contains("text") || fn.contains("ruler")) {
                            XposedBridge.log(TAG + ":   FIELD " + f.getName()
                                + " : " + f.getType().getSimpleName()
                                + (java.lang.reflect.Modifier.isStatic(f.getModifiers()) ? " [static]" : ""));
                        }
                    }
                } catch (Throwable t) { /* class load failed, skip */ }
            }
            dexFile.close();
            XposedBridge.log(TAG + ": SCAN DONE: " + found + " matching classes in " + lpparam.packageName);
        } catch (Throwable t) {
            XposedBridge.log(TAG + ": SCAN via DexFile failed: " + t.getMessage());

            // Fallback: пробуем известные классы вручную
            String[] guesses = {
                "ecarx.launcher3.widget.radio.RadioRulerView",
                "ecarx.launcher3.widget.RadioRulerView",
                "ecarx.launcher3.view.RadioRulerView",
                "ecarx.launcher3.radio.RadioRulerView",
                "ecarx.launcher3.widget.media.RadioRulerView",
                "ecarx.launcher3.widget.media.RadioWidgetView",
                "ecarx.launcher3.widget.MediaWidgetRadioView",
                "ecarx.launcher3.appwidget.RadioRulerView",
                "ecarx.launcher3.LauncherAppWidgetHostView",
                "ecarx.xsf.widget.view.RadioRulerView",
                "ecarx.xsf.widget.view.FmRulerView",
                "ecarx.xsf.widget.remoteviews.FoldWidgetRemoteViews",
            };
            for (String cn : guesses) {
                try {
                    Class<?> cls = lpparam.classLoader.loadClass(cn);
                    XposedBridge.log(TAG + ": SCAN FOUND: " + cn);
                    for (Method m : cls.getDeclaredMethods()) {
                        Class<?>[] params = m.getParameterTypes();
                        StringBuilder sb = new StringBuilder();
                        for (Class<?> p : params) sb.append(p.getSimpleName()).append(",");
                        XposedBridge.log(TAG + ":   -> " + m.getName() + "(" + sb + ")");
                    }
                } catch (Throwable t2) { /* not found */ }
            }
        }

        // 2. Хукаем TextView.setText — на случай если виджет использует обычный TextView
        try {
                ...
                new XC_MethodHook() {
                    ...
                    protected void beforeHookedMethod(MethodHookParam param) {
                        try {
                            ...
                            String found = StationsDatabase.findName(freq);
                            if (found == null) return;
                            // Логируем стек вызовов чтобы понять КТО ставит текст
                            String caller = "";
                            StackTraceElement[] stack = Thread.currentThread().getStackTrace();
                            for (int i = 4; i < Math.min(stack.length, 10); i++) {
                                String cn = stack[i].getClassName();
                                if (!cn.startsWith("android.") && !cn.startsWith("com.android.")
                                    && !cn.contains("xposed") && !cn.contains("belgee")) {
                                    caller = cn + "." + stack[i].getMethodName()
                                        + ":" + stack[i].getLineNumber();
                                    break;
                                }
                            }
                            XposedBridge.log(TAG + ": >>> TEXTVIEW.setText '" + text
                                + "' freq=" + freq + " caller=" + caller);
                            param.args[0] = found + " " + freq;
                        } catch (Throwable t) { /* skip */ }
                    }
                });
            XposedBridge.log(TAG + ": ✓ hooked TextView.setText(4-arg) in " + lpparam.packageName);
        } catch (Throwable t) {
            XposedBridge.log(TAG + ": TextView.setText hook failed: " + t.getMessage());
        }

        XposedBridge.log(TAG + ": ===== DEEP SCAN " + lpparam.packageName + " COMPLETE =====");
    }
    ...
    /**
     * v24: Хукаем AppWidgetHostView.updateAppWidget(RemoteViews) в launcher.
     * Когда launcher получает RemoteViews от NSMediaWidget, проходим по mActions
     * и ищем ReflectionAction с текстом "FM XX.X" для подмены.
     */
    private void hookLauncherUpdateAppWidget(XC_LoadPackage.LoadPackageParam lpparam) {
        // 1. Хукаем LauncherAppWidgetHostView.updateAppWidget
        try {
            Class<?> hostViewClass = lpparam.classLoader.loadClass(
                "ecarx.launcher3.LauncherAppWidgetHostView");
            XposedHelpers.findAndHookMethod(hostViewClass, "updateAppWidget",
                ...
                new XC_MethodHook() {
                    ...
                    protected void beforeHookedMethod(MethodHookParam param) {
                        try {
                            ...
                            if (rv == null) return;
                            modifyRemoteViewsActions(rv);
                        } catch (Throwable t) {
                            XposedBridge.log(TAG + ": updateAppWidget hook error: " + t.getMessage());
                        }
                    }
                });
            XposedBridge.log(TAG + ": ✓ hooked LauncherAppWidgetHostView.updateAppWidget");
        } catch (Throwable t) {
            XposedBridge.log(TAG + ": LauncherAppWidgetHostView not found: " + t.getMessage());
        }

        // 2. Также хукаем ScalesLauncherAppWidgetHostView.updateAppWidget
        try {
            Class<?> scalesClass = lpparam.classLoader.loadClass(
                "ecarx.launcher3.ScalesLauncherAppWidgetHostView");
            XposedHelpers.findAndHookMethod(scalesClass, "updateAppWidget",
                android.widget.RemoteViews.class,
                new XC_MethodHook() {
                    @Override
                    protected void beforeHookedMethod(MethodHookParam param) {
            ...
            XposedBridge.log(TAG + ": ✓ hooked ScalesLauncherAppWidgetHostView.updateAppWidget");
        } catch (Throwable t) { }

        // 3. Хукаем стандартный AppWidgetHostView.updateAppWidget
        // ПОСЛЕ применения RemoteViews — ищем RadioRulerView и подменяем pointerValueStr
        try {
                ...
                new XC_MethodHook() {
                    ...
                    protected void afterHookedMethod(MethodHookParam param) {
                        try {
                            android.widget.RemoteViews rv = (android.widget.RemoteViews) param.args[0];
                            if (rv == null) return;
                            String pkg = rv.getPackage();
                            if (!"ecarx.xsf.widget".equals(pkg)) return;

                            // Ищем RadioRulerView в дочерних View
                            ...
                            // Хукаем RadioRulerView один раз через его classLoader
                            if (!radioRulerHooked) {
                                radioRulerHooked = true;
                                Class<?> rrClass = radioRuler.getClass();
                                XposedBridge.log(TAG + ": ✓ FOUND RadioRulerView class: " + rrClass.getName());

                                // Логируем поля
                                for (java.lang.reflect.Field f : rrClass.getDeclaredFields()) {
                                    if (f.getType() == String.class || f.getType() == float.class || f.getType() == int.class) {
                                        XposedBridge.log(TAG + ": RR field: " + f.getType().getSimpleName() + " " + f.getName());
                                    }
                                }
                                        ...
                                        new XC_MethodHook() {
                                            ...
                                            protected void afterHookedMethod(MethodHookParam p) {
                                                try {
                                                    ...
                                                    // Подменяем pointerValueStr
                                                    try {
                                                        XposedHelpers.setObjectField(p.thisObject, "pointerValueStr", name);
                                                        ((android.view.View) p.thisObject).invalidate();
                                                        XposedBridge.log(TAG + ": >>> pointerValueStr -> '" + name + "' (freq=" + freq + ")");
                                                    } catch (Throwable t1) {
                                                        XposedBridge.log(TAG + ": pointerValueStr set failed: " + t1.getMessage());
                                                    }
                                                } catch (Throwable t) { }
                                            }
                                        });
                                    XposedBridge.log(TAG + ": ✓ hooked RadioRulerView.setRadioFreq via runtime class");
                                } catch (Throwable t) {
                                    XposedBridge.log(TAG + ": setRadioFreq hook failed: " + t.getMessage());
                                }
                            }

                            // Также подменяем pointerValueStr прямо сейчас
                            try {
                                ...
                                if (pvs != null && pvs.matches("\\d+\\.\\d+")) {
                                    ...
                                    if (name != null) {
                                        XposedHelpers.setObjectField(radioRuler, "pointerValueStr", name);
                                        radioRuler.invalidate();
                                        XposedBridge.log(TAG + ": >>> DIRECT pointerValueStr '" + pvs + "' -> '" + name + "'");
                                    }
                                }
                            } catch (Throwable t) {
                                XposedBridge.log(TAG + ": direct pointerValueStr failed: " + t.getMessage());
                            }

                        } catch (Throwable t) { }
                    }
                });
            XposedBridge.log(TAG + ": ✓ hooked AppWidgetHostView.updateAppWidget (after)");
        } catch (Throwable t) { }
    }

    /**
     * v27: Перехватываем setRadioFreq + инжектируем имя станции
     * 
     * ИЗВЕСТНЫЕ viewId из дампа actions:
     * 0x7f08014a — RadioRulerView (шкала частот)
     * 0x7f0801c8 — tv_lyric (РАБОТАЕТ но это заголовок "Мультимедиа" сверху!)
     * 0x7f0801c7 — кликабельный view (SetOnClickPendingIntent)
     * 0x7f0801c5 — tv_lyric_layout (VISIBLE)
     * 0x7f0801c4 — кликабельный view
     * 0x7f0801bf — tv_lyric_name_layout (GONE)
     * 0x7f0801be — кликабельный view
     * 0x7f0800e3 — какой-то view (visibility=0)
     * 
     * Стратегия: пишем имя в tv_lyric_name_layout area
     */
    private static final int VIEW_ID_RADIO_RULER = 0x7f08014a;

    @SuppressWarnings("unchecked")
    private void modifyRemoteViewsActions(android.widget.RemoteViews rv) {
        try {
            String pkg = rv.getPackage();
            if (!"ecarx.xsf.widget".equals(pkg)) return;

            java.util.ArrayList<Object> actions = (java.util.ArrayList<Object>)
                XposedHelpers.getObjectField(rv, "mActions");
            if (actions == null || actions.isEmpty()) return;

            String freqFound = null;
            String stationName = null;

            for (Object action : actions) {
                if (action == null) continue;
                String actionClass = action.getClass().getSimpleName();
                if (!"ReflectionAction".equals(actionClass)) continue;
                try {
                    String methodName = (String) XposedHelpers.getObjectField(action, "methodName");
                    if (!"setRadioFreq".equals(methodName)) continue;
                    Object value = XposedHelpers.getObjectField(action, "value");
                    if (value == null) continue;
                    String val = value.toString();
                    int comma = val.indexOf(',');
                    if (comma >= 0 && comma < val.length() - 1) {
                        freqFound = val.substring(comma + 1);
                        stationName = StationsDatabase.findName(freqFound);
                    }
                } catch (Throwable t) { }
            }

            if (stationName != null) {
                // v32: НЕ пишем в tv_mask — он перекрывает шкалу серым фоном!
                // pointerValueStr подменяется в хуке setRadioFreq (ниже)
                // Здесь только логируем
                XposedBridge.log(TAG + ": >>> RV freq=" + freqFound + " name=" + stationName);
            }
        } catch (Throwable t) {
            XposedBridge.log(TAG + ": modifyRV error: " + t.getMessage());
        }
    }

    /**
     * v26: Хукаем RadioRulerView.setRadioFreq в launcher.
     * Этот кастомный View рисует шкалу частот и текст "FM XX.X" через Canvas.
     * setRadioFreq(String typeAndFreq) — вызывается через RemoteViews action.
     * Формат: "0,88.4" (band,freq) или "1,1269" (AM)
     *
     * Стратегия: после вызова setRadioFreq дорисовываем имя станции
     * или подменяем текст который рисуется.
     */
    private void hookLauncherRadioRulerView(XC_LoadPackage.LoadPackageParam lpparam) {
        String[] candidates = {
            "ecarx.xsf.widget.custom.RadioRulerView",  // ← ТОЧНОЕ имя из view dump!
            "ecarx.launcher3.widget.RadioRulerView",
            "ecarx.launcher3.smartcard.widget.RadioRulerView",
            "ecarx.launcher3.view.RadioRulerView",
            "ecarx.launcher3.RadioRulerView",
        };

        Class<?> radioRulerClass = null;
        for (String cn : candidates) {
            try {
                radioRulerClass = lpparam.classLoader.loadClass(cn);
                XposedBridge.log(TAG + ": ✓ found RadioRulerView: " + cn);
                break;
            } catch (ClassNotFoundException e) {
                XposedBridge.log(TAG + ": RadioRulerView not at: " + cn);
            }
        }

        if (radioRulerClass == null) {
            XposedBridge.log(TAG + ": RadioRulerView not found by any name");
            return;
        }

        hookRadioRulerSetRadioFreq(radioRulerClass);
    }

    /**
     * v29: Хукаем setRadioFreq на RadioRulerView — подменяем pointerValueStr
     * 
     * Из логов RadioRulerView:
     *   setRadioFreq typeAndFreq : 0,92.0
     *   pointerValueStr : 92.0
     *   ---drawUnderNum-----setTextSize : 28
     * 
     * pointerValueStr — это ТЕКСТ который рисуется через Canvas.drawText!
     * Подменяем его на "Радио Маяк" после setRadioFreq.
     */
    private void hookRadioRulerSetRadioFreq(Class<?> cls) {
        // Сначала логируем все поля класса
        try {
            java.lang.reflect.Field[] fields = cls.getDeclaredFields();
            for (java.lang.reflect.Field f : fields) {
                XposedBridge.log(TAG + ": RadioRulerView field: " + f.getType().getSimpleName()
                    + " " + f.getName());
            }
        } catch (Throwable t) { }

        try {
            XposedHelpers.findAndHookMethod(cls, "setRadioFreq", String.class,
                new XC_MethodHook() {
                    @Override
                    protected void afterHookedMethod(MethodHookParam param) {
                        try {
                            String typeAndFreq = (String) param.args[0];
                            if (typeAndFreq == null) return;
                            int comma = typeAndFreq.indexOf(',');
                            if (comma < 0) return;
                            String freq = typeAndFreq.substring(comma + 1);
                            String stationName = StationsDatabase.findName(freq);

                            if (stationName == null) return;

                            // Пробуем подменить pointerValueStr
                            try {
                                String oldVal = (String) XposedHelpers.getObjectField(
                                    param.thisObject, "pointerValueStr");
                                XposedHelpers.setObjectField(param.thisObject,
                                    "pointerValueStr", stationName);
                                XposedBridge.log(TAG + ": >>> RadioRuler pointerValueStr '"
                                    + oldVal + "' -> '" + stationName + "'");
                            } catch (Throwable t) {
                                XposedBridge.log(TAG + ": pointerValueStr not found: " + t.getMessage());
                            }

                            // Также пробуем другие возможные имена полей
                            String[] fieldNames = {"mPointerValueStr", "mFreqText", "freqText",
                                "mText", "valueStr", "mValueStr", "displayText"};
                            for (String fn : fieldNames) {
                                try {
                                    XposedHelpers.setObjectField(param.thisObject, fn, stationName);
                                    XposedBridge.log(TAG + ": >>> RadioRuler " + fn + " -> '" + stationName + "'");
                                } catch (Throwable t) { /* field doesn't exist */ }
                            }
                            ...
                            XposedBridge.log(TAG + ": setRadioFreq hook error: " + t.getMessage());
                        }
                    }
                });
            XposedBridge.log(TAG + ": ✓ hooked " + cls.getName() + ".setRadioFreq(String)");
        } catch (Throwable t) {
            XposedBridge.log(TAG + ": setRadioFreq hook failed on " + cls.getName() + ": " + t.getMessage());
            // Логируем все методы для диагностики
            try {
                for (java.lang.reflect.Method m : cls.getDeclaredMethods()) {
                    XposedBridge.log(TAG + ": RadioRulerView method: " + m.getName()
    ...
    /**
     * v26: Рекурсивно ищем TextView в ViewGroup для имени станции
     */
    private void findAndSetTextView(android.view.ViewGroup vg, String text, int depth) {
        if (depth > 3) return;
        for (int i = 0; i < vg.getChildCount(); i++) {
            ...
            if (child instanceof android.widget.TextView) {
                ...
                int id = tv.getId();
                XposedBridge.log(TAG + ": >>> LAUNCHER TV id=0x" + String.format("%08x", id)
                    + " text='" + current + "' vis=" + tv.getVisibility());
            }
            if (child instanceof android.view.ViewGroup) {
                findAndSetTextView((android.view.ViewGroup) child, text, depth + 1);
            }
        }
    }
    ...
    private void hookRemoteViewsSetTextViewText(XC_LoadPackage.LoadPackageParam lpparam) {
        try {
            ...
            // 1. setTextViewText(int viewId, CharSequence text)
            try {
                    ...
                    new XC_MethodHook() {
                        ...
                        protected void beforeHookedMethod(MethodHookParam param) {
                            try {
                                ...
                                // v21: Логируем ВСЕ вызовы (первые 200)
                                if (logCount < 200) {
                                    logCount++;
                                    XposedBridge.log(TAG + ": >>> RV.setTextViewText(0x"
                                        + String.format("%08x", viewId) + ", \"" + text + "\")");
                                }

                                // Подмена если есть частота
                    ...
                    new XC_MethodHook() {
                        ...
                        protected void beforeHookedMethod(MethodHookParam param) {
                            try {
                                ...
                                // v21: Логируем ВСЕ вызовы setText
                                if ("setText".equals(methodName)) {
                                    XposedBridge.log(TAG + ": >>> RV.setCharSequence(0x"
                                        + String.format("%08x", viewId) + ", setText, \"" + text + "\")");
                                }
                                ...
                                XposedBridge.log(TAG + ": >>> RemoteViews.setCharSequence(0x"
                                    + String.format("%08x", viewId) + ", setText, \""
                                    + text + "\") -> \"" + newText + "\"");
                            } catch (Throwable t) { /* skip */ }
                        }
                    });
    ...
    private void tryKnownWidgetClasses(XC_LoadPackage.LoadPackageParam lpparam) {
        ...
        try {
            ...
            hookUpdateContentTitle(rv, "UnfoldRadioWidgetRemoteViews");
        } catch (Throwable t) {
            XposedBridge.log(TAG + ": UnfoldRadioWidgetRemoteViews not found: " + t.getMessage());
        }

        // 3) FoldWidgetRemoteViews - для свёрнутого вида на ГЛАВНОМ ЭКРАНЕ
        try {
            final Class<?> rv = cl.loadClass("ecarx.xsf.widget.remoteviews.FoldWidgetRemoteViews");
            XposedBridge.log(TAG + ": ✓ found FoldWidgetRemoteViews");
            for (Method m : rv.getDeclaredMethods()) {
                Class<?>[] params = m.getParameterTypes();
                StringBuilder sb = new StringBuilder();
                for (Class<?> p : params) sb.append(p.getSimpleName()).append(",");
                XposedBridge.log(TAG + ":   FOLD METHOD: " + m.getName() + "(" + sb + ") ret=" + m.getReturnType().getSimpleName());
            }
            hookRemoteViewsSetters(rv);
            hookUpdateContentTitle(rv, "FoldWidgetRemoteViews");

            // v20: ХУКАЕМ ВСЕ МЕТОДЫ FoldWidgetRemoteViews — логируем входы
            for (final Method m : rv.getDeclaredMethods()) {
                final String mn = m.getName();
                final int paramCount = m.getParameterCount();
                if (mn.equals("<init>") || mn.equals("<clinit>")) continue;
                try {
                    XposedBridge.hookMethod(m, new XC_MethodHook() {
                        private volatile int callCount = 0;
                        private final ThreadLocal<Object[]> saved = new ThreadLocal<>();

                        @Override
                        protected void beforeHookedMethod(MethodHookParam param) {
                            try {
                                callCount++;
                                if (callCount > 50) return; // не спамим

                                ClassLoader cl2 = param.thisObject.getClass().getClassLoader();
                                Class<?> holderClass;
                                try {
                                    holderClass = cl2.loadClass("ecarx.xsf.widget.holder.MusicPlayInfoHolder");
                                } catch (Throwable t) { return; }

                                Object holder = null;
                                try { holder = XposedHelpers.callStaticMethod(holderClass, "getInstance"); }
                                catch (Throwable t) { }
                                if (holder == null) {
                                    for (java.lang.reflect.Field f : holderClass.getDeclaredFields()) {
                                        if (java.lang.reflect.Modifier.isStatic(f.getModifiers())) {
                                            f.setAccessible(true);
                                            try {
                                                Object val = f.get(null);
                                                if (val != null && holderClass.isInstance(val)) { holder = val; break; }
                                            } catch (Throwable t) { }
                                        }
                                    }
                                }
                                ...
                                String freq = null;
                                try { freq = (String) XposedHelpers.getObjectField(holder, "mLastRadioFreq"); }
                                catch (Throwable t) { return; }
                                if (freq == null || freq.isEmpty() || freq.length() > 10) return;

                                String found = StationsDatabase.findName(freq);
                                if (found == null) return;

                                String newFreq = found + " " + freq;
                                XposedHelpers.setObjectField(holder, "mLastRadioFreq", newFreq);

                                StringBuilder args = new StringBuilder();
                                if (param.args != null) for (Object a : param.args) args.append(a).append(",");
                                XposedBridge.log(TAG + ": >>> FOLD." + mn + "(" + args + ") freq '"
                                    + freq + "' -> '" + newFreq + "'");

                                saved.set(new Object[]{freq, holder});
                            } catch (Throwable t) { /* skip */ }
                        }
                        ...
                        protected void afterHookedMethod(MethodHookParam param) {
                            try {
                                ...
                                if (s != null) {
                                    saved.remove();
                                    XposedHelpers.setObjectField(s[1], "mLastRadioFreq", (String) s[0]);
                                }
                            } catch (Throwable t) { }
                        }
                    });
                    XposedBridge.log(TAG + ": ✓ hooked FOLD." + mn + "(" + paramCount + ")");
                } catch (Throwable t) {
                    XposedBridge.log(TAG + ": ✗ FOLD." + mn + " hook failed: " + t.getMessage());
                }
            }
    ...
    /**
     * v16: Хукаем updateContentTitle() на классах виджета (Unfold/Fold RemoteViews).
     *
     * Этот метод формирует текст "FM 100.4" и ставит его на RemoteViews.
     * Код в vdex — мы не знаем деталей, но можем:
     *
     * 1) afterHookedMethod: после выполнения — через рефлексию достать
     *    приватное поле RemoteViews и модифицировать actions.
     *
     * 2) beforeHookedMethod: подменить поле mLastRadioFreq в MusicPlayInfoHolder
     *    чтобы updateContentTitle использовал "Имя freq" вместо "freq".
     *
     * Используем подход 2: перед updateContentTitle подменяем mLastRadioFreq
     * в MusicPlayInfoHolder на "Имя freq", а после — возвращаем обратно.
     */
    private void hookUpdateContentTitle(Class<?> rvClass, final String className) {
        // ВАЖНО: Kotlin компилирует fun updateContentTitle(needRefresh: Boolean = false) в:
        //   updateContentTitle()              — bridge без аргументов
        //   updateContentTitle(boolean)       — реальный метод
        //   updateContentTitle$default(...)   — synthetic default
        // Нужно хукнуть ВСЕ варианты!

        // 1. Сначала дампим ВСЕ методы содержащие "updateContentTitle" или "refreshTitle"
        java.util.List<Method> targets = new java.util.ArrayList<>();
        for (Method m : rvClass.getDeclaredMethods()) {
            String mn = m.getName();
            if (mn.contains("updateContentTitle") || mn.contains("ContentTitle")
                    || mn.contains("refreshTitle") || mn.contains("updateContent")) {
                Class<?>[] params = m.getParameterTypes();
                StringBuilder sb = new StringBuilder();
                for (Class<?> p : params) sb.append(p.getSimpleName()).append(", ");
                XposedBridge.log(TAG + ":   METHOD " + className + "." + mn
                    + "(" + sb.toString() + ") ret=" + m.getReturnType().getSimpleName());
                targets.add(m);
            }
        }

        if (targets.isEmpty()) {
            XposedBridge.log(TAG + ": " + className + ".updateContentTitle — NO methods found");
            // Дампим ВСЕ методы класса
            for (Method m : rvClass.getDeclaredMethods()) {
                XposedBridge.log(TAG + ":   ALL: " + m.getName() + "(" + m.getParameterCount() + " args)");
            }
            return;
        }

        // 2. Хукаем ВСЕ найденные методы
        for (final Method targetMethod : targets) {
            final String methodSig = targetMethod.getName() + "(" + targetMethod.getParameterCount() + " args)";
            try {
                XposedBridge.hookMethod(targetMethod, new XC_MethodHook() {
                    private final ThreadLocal<Object[]> saved = new ThreadLocal<>();
                    private volatile boolean dumpedOnce = false;
                    ...
                    protected void beforeHookedMethod(MethodHookParam param) {
                        try {
                            ...
                            XposedBridge.log(TAG + ": >>> " + className + "." + methodSig
                                + " ENTER args=[" + argStr + "]");

                            // Дамп полей this (один раз)
                            if (!dumpedOnce) {
                                dumpedOnce = true;
                                Object self = param.thisObject;
                                if (self != null) {
                                    XposedBridge.log(TAG + ":   this class: " + self.getClass().getName());
                                    // Дампим поля this и parent
                                    for (Class<?> c = self.getClass(); c != null && c != Object.class; c = c.getSuperclass()) {
                                        for (java.lang.reflect.Field f : c.getDeclaredFields()) {
                                            f.setAccessible(true);
                                            try {
                                                Object val = f.get(self);
                                                String vs = (val != null) ? val.getClass().getSimpleName() + "=" +
                                                    (val.toString().length() > 80 ? val.toString().substring(0, 80) : val.toString()) : "null";
                                                XposedBridge.log(TAG + ":   " + c.getSimpleName() + "." + f.getName() + " : " + vs);
                                            } catch (Throwable t) { /* skip */ }
                                        }
                                    }
                                }
                            ...
                            // Ищем MusicPlayInfoHolder
                            ClassLoader cl = param.thisObject.getClass().getClassLoader();
                            Class<?> holderClass = cl.loadClass("ecarx.xsf.widget.holder.MusicPlayInfoHolder");
                            Object holder = null;
                            ...
                            if (holder == null) {
                                for (java.lang.reflect.Field f : holderClass.getDeclaredFields()) {
                                    if (java.lang.reflect.Modifier.isStatic(f.getModifiers())) {
                                        ...
                                        try {
                                            Object val = f.get(null);
                                            if (val != null && holderClass.isInstance(val)) {
                                                holder = val;
                                                break;
                                            }
                                        } catch (Throwable t) { }
                            ...
                            if (holder == null) {
                                XposedBridge.log(TAG + ":   ✗ holder NULL");
                                return;
                            }
                            ...
                            String freq = null;
                            try {
                                freq = (String) XposedHelpers.getObjectField(holder, "mLastRadioFreq");
                            } catch (Throwable t) {
                                try {
                                    Object r = XposedHelpers.callMethod(holder, "getLastRadioFreq");
                                    freq = r != null ? r.toString() : null;
                            ...
                            XposedBridge.log(TAG + ": >>> " + className + "." + methodSig
                                + " mLastRadioFreq '" + freq + "' -> '" + newFreq + "'");
                        } catch (Throwable t) {
                            XposedBridge.log(TAG + ": " + methodSig + " EXCEPTION: " + t);
                        }
                    }

                    @Override
                    protected void afterHookedMethod(MethodHookParam param) {
                        // v24: ОТКАТЫВАЕМ mLastRadioFreq чтобы не ломать избранное/шкалу
                        try {
                            Object[] s = saved.get();
                            if (s != null) {
                                saved.remove();
                ...
                XposedBridge.log(TAG + ": ✓ hooked " + className + "." + methodSig);
            } catch (Throwable t) {
                XposedBridge.log(TAG + ": ✗ hook failed " + className + "." + methodSig + ": " + t.getMessage());
            }
        }
    }
