...
public class MainActivity extends Activity {
    ...
    private static final String DUMP_TREE_MARKER_PATH = DIR_PATH + "/dump_tree_request.txt";

    private void requestWidgetTreeDump() {
        try (FileWriter w = new FileWriter(DUMP_TREE_MARKER_PATH)) { w.write("1"); }
        catch (Exception e) { Log.w("RadioName", e.getMessage(), e); }
        Toast.makeText(this, "Запрошен дамп дерева виджета — переключи станцию на виджете, потом смотри лог",
            Toast.LENGTH_LONG).show();
    }

    private void saveFontScale(float scale) {
        try (FileWriter w = new FileWriter(FONT_SCALE_PATH)) { w.write(String.valueOf(scale)); }
        catch (Exception e) { Log.w("RadioName", e.getMessage(), e); }
        String extra = (currentDisplayStyle == 0)
            ? "\n(для стиля «Только имя» размер всегда оригинальный — переключи на «Частота + имя» чтобы увидеть эффект)"
            : "";
        Toast.makeText(this, "Размер шрифта: " + (int)(scale * 100) + "% от исходного" + extra +
            "\nПереключи станцию на виджете чтобы увидеть", Toast.LENGTH_LONG).show();
    }
    ...
    private void buildUI() {
        ...
        // Размер шрифта в виджете — несколько пресетов, тыкаешь и сразу видно
        // на виджете (обновляется за пару секунд, без пересборки APK).
        LinearLayout fontSizeLabelRow = new LinearLayout(this);
        fontSizeLabelRow.setOrientation(LinearLayout.HORIZONTAL);
        fontSizeLabelRow.setPadding(0, 8, 0, 4);
        TextView fsText = new TextView(this); fsText.setText("🔤  Размер шрифта в виджете:");
        fsText.setTextSize(16); fsText.setTextColor(TEXT_SECONDARY);
        fontSizeLabelRow.addView(fsText);
        settingsCard.addView(fontSizeLabelRow);

        LinearLayout fontSizeRow = new LinearLayout(this);
        fontSizeRow.setOrientation(LinearLayout.HORIZONTAL);
        fontSizeRow.setPadding(0, 0, 0, 8);
        float[] fontPresets = {0.85f, 0.87f, 0.89f, 0.90f, 0.92f, 0.94f, 0.95f};
        String[] fontLabels = {"85%", "87%", "89%", "90%", "92%", "94%", "95%"};
        for (int i = 0; i < fontPresets.length; i++) {
            final float presetValue = fontPresets[i];
            Button b = styledBtn(fontLabels[i], BTN_BG, v -> saveFontScale(presetValue));
            fontSizeRow.addView(b);
        }
        settingsCard.addView(fontSizeRow);
        ...
        debugRow.setPadding(0, 4, 0, 8);
        debugRow.addView(styledBtn("🔍 Дамп дерева виджета", BTN_BG, v -> requestWidgetTreeDump()));
        advancedContainer.addView(debugRow);
