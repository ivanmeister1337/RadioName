...
public class MainActivity extends Activity {
    ...
    private DbSnapshot snapshotUserData() {
        ...
        // Запоминаем ВЫБРАННЫЙ пользователем город по имени — раньше это НЕ
        // сохранялось, и любое обновление базы (локальное или через GitHub)
        // могло тихо переключить город на то, что записано в НОВОЙ базе
        // (например на Москву), даже если пользователь сам выбрал другой.
    ...
    private void toggleGps() {
        ...
        // Раньше запись не проверялась — если она молча не удавалась
        // (например, нет прав на запись), тумблер выглядел включённым
        // в этой сессии, но при следующем запуске "сам сбрасывался",
        // потому что на диске ничего на самом деле не сохранилось.
