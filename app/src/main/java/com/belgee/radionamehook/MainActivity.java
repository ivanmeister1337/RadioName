...
public class MainActivity extends Activity {
    ...
    private int currentCityId = 0; // 0 = город ещё не выбран
    // Кандидат на переключение города по GPS — переключаем только если ОДИН
    // И ТОТ ЖЕ город определяется ближайшим 2 раза подряд. Единичный "глюк"
    // координат (например отражение сигнала) не должен вызывать мгновенное
    // переключение — тогда бы могло на секунду проскочить название станции
    // из совсем другого региона (замечено: "Красная Поляна" при нахождении в СПб).
    private int pendingSwitchCityId = 0;
    private String currentCityName = "Не выбран";
    ...
    private static class DbSnapshot {
        ...
        String selectedCityName = null; // выбранный пользователем город — по ИМЕНИ,
                                         // т.к. id могут отличаться между версиями базы
    }
    ...
    private DbSnapshot snapshotUserData() {
        ...
        // Запоминаем выбранный пользователем город по имени — так он переживёт
        // замену базы (локальную или через онлайн-обновление), независимо от
        // того, что записано в settings самой новой базы.
        if (currentCityId != 0) snap.selectedCityName = currentCityName;
    ...
    private int[] restoreUserData(DbSnapshot snap) {
        ...
        // Восстанавливаем выбранный город — ищем по ИМЕНИ в новой базе (id могли
        // измениться). Если город с таким именем есть — переключаемся на него,
        // независимо от того что записано в settings самой новой базы.
        if (snap.selectedCityName != null) {
            try {
                Cursor c = db.rawQuery("SELECT id FROM cities WHERE name=?",
                    new String[]{snap.selectedCityName});
                if (c.moveToFirst()) {
                    int newCityId = c.getInt(0);
                    db.execSQL("INSERT OR REPLACE INTO settings(key,value) VALUES('current_city_id',?)",
                        new Object[]{String.valueOf(newCityId)});
                }
                c.close();
            } catch (Exception e) { Log.w("RadioName", e.getMessage(), e); }
        }
    ...
    /**
     * Полный сброс — в отличие от resetDatabase() НЕ сохраняет и не
     * восстанавливает пользовательские переименования и кастомные города.
     * Просто заменяет базу на встроенную "как есть", с нуля.
     */
    private void fullReset() {
        try {
            ...
            loadSettings();
            refreshList();
            Toast.makeText(this, "✓ Полный сброс выполнен — база как при первой установке",
                Toast.LENGTH_LONG).show();
        } catch (Exception e) {
            Toast.makeText(this, "Ошибка: " + e.getMessage(), Toast.LENGTH_LONG).show();
        }
    ...
    /** Оборачивает фон в ripple — тактильная отдача при нажатии вместо мгновенной смены цвета. */
    private android.graphics.drawable.Drawable rippleBg(int baseColor) {
        android.graphics.drawable.GradientDrawable base = btnBg(baseColor);
        int rippleColor = 0x33FFFFFF; // полупрозрачный белый поверх любого базового цвета
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
            return new android.graphics.drawable.RippleDrawable(
                android.content.res.ColorStateList.valueOf(rippleColor), base, base);
        }
        return base;
    }
    ...
    private Button styledBtn(String text, int bgColor, View.OnClickListener listener) {
        ...
        btn.setBackground(rippleBg(bgColor));
        btn.setStateListAnimator(null); // убираем стандартную тень Android-кнопки, у нас своя эстетика
    ...
    /** Версия приложения (versionName + versionCode из build.gradle), для отображения в UI. */
    private String getAppVersionString() {
        try {
            android.content.pm.PackageInfo pi = getPackageManager().getPackageInfo(getPackageName(), 0);
            return "v" + pi.versionName + " (build " + pi.versionCode + ")";
        } catch (Exception e) {
            return "v?";
        }
    }
    ...
    private LinearLayout card(LinearLayout parent) {
        ...
        // Лёгкий подъём над фоном — тень вместо плоского прямоугольника.
    ...
    private void buildUI() {
        ...
        // Версия приложения — читаем динамически из PackageInfo, а не
        // хардкодим строку, чтобы не забывать обновлять вручную при каждой сборке.
        TextView versionLabel = new TextView(this);
        versionLabel.setText(getAppVersionString());
        versionLabel.setTextSize(11); versionLabel.setTextColor(TEXT_SECONDARY);
        versionLabel.setGravity(Gravity.CENTER);
        versionLabel.setPadding(0, 0, 0, 14);
        ...
        cityLabel.setTextColor(currentCityId == 0 ? TEXT_WARN : ACCENT);
        ...
        // Второстепенные блоки (онлайн-обновление, диагностика LSPosed, сброс/
        // экспорт/импорт), не нужные для повседневного использования — прячем
        // за один переключатель, чтобы не перегружать главный экран.
        ...
        dangerRow.setGravity(Gravity.CENTER);
        dangerRow.setPadding(0, 4, 0, 8);
        dangerRow.addView(styledBtn("🗑 Полный сброс (стереть все мои названия)", 0xFF8B0000, v -> new AlertDialog.Builder(this)
            .setTitle("Полный сброс?")
            .setMessage("⚠ Все твои переименования станций и добавленные вручную города БУДУТ УДАЛЕНЫ без возможности восстановления.\n\nБаза вернётся в состояние как при первой установке.")
            .setPositiveButton("Стереть всё", (d, w) -> fullReset())
            .setNegativeButton("Отмена", null).show()));
        advancedContainer.addView(dangerRow);
    ...
    private void updateGpsLabel() {
        ...
        if (gpsToggleBtn != null) {
            ...
            gpsToggleBtn.setBackground(rippleBg(autoGpsEnabled ? 0xFF8B0000 : BTN_ACCENT));
        }
    }
    ...
    private void toggleGps() {
        ...
        boolean saved = false;
        try (FileWriter w = new FileWriter(DIR_PATH + "/gps_auto.txt")) {
            w.write(autoGpsEnabled ? "1" : "0");
        } catch (Exception e) { Log.w("RadioName", e.getMessage(), e); }
        // Проверяем что запись реально прошла (например, могло не хватить
        // прав на запись) — иначе тумблер выглядит включённым в этой сессии,
        // но при следующем запуске состояние не восстановится с диска.
        try (BufferedReader r = new BufferedReader(new FileReader(DIR_PATH + "/gps_auto.txt"))) {
            String written = r.readLine();
            saved = (autoGpsEnabled ? "1" : "0").equals(written);
        } catch (Exception e) { Log.w("RadioName", e.getMessage(), e); }
        if (!saved) {
            Toast.makeText(this, "⚠ Не удалось сохранить настройку GPS — проверь права на запись",
                Toast.LENGTH_LONG).show();
        }
        updateGpsLabel();
        if (autoGpsEnabled) {
    ...
    private void checkNearestCity(double lat, double lon) {
        ...
        try {
            ...
            runOnUiThread(() -> {
                ...
                boolean candidateOk = fNearestId != currentCityId && fMinDist < 200
                    && (fCurrentDist == Double.MAX_VALUE || fCurrentDist - fMinDist > 8);

                if (!candidateOk) {
                    // Условие не выполняется — сбрасываем отложенного кандидата,
                    // если он был (например, GPS вернулся к прежнему городу).
                    pendingSwitchCityId = 0;
                }

                boolean shouldSwitch = false;
                if (candidateOk) {
                    if (pendingSwitchCityId == fNearestId) {
                        // Тот же кандидат второй раз подряд — подтверждено, переключаем.
                        shouldSwitch = true;
                        pendingSwitchCityId = 0;
                    } else {
                        // Первый раз видим этого кандидата — запоминаем, но пока
                        // НЕ переключаем. Подтвердится на следующем опросе — тогда применим.
                        pendingSwitchCityId = fNearestId;
                    }
                }
    ...
    private void editCurrentStation() {
        if (currentCityId == 0) {
            Toast.makeText(this, "Сначала выбери город", Toast.LENGTH_SHORT).show();
            return;
        }
        try {
            File f = new File(FREQ_PATH);
    ...
    private void saveStation(String freq, String name) {
        if (db == null) return;
        try {
            // Определяем band так же, как везде в коде: частота с точкой
            // (103.4) — FM, без точки (675) — AM.
    ...
    /**
     * Имена городов из ВСТРОЕННОЙ (заводской) базы в assets — используется чтобы
     * отличить кастомные (добавленные пользователем вручную) города от обычных.
     * Раньше для этого использовалась проверка "id > 99" — стала неверной, когда
     * база выросла с 99 до 1500+ городов с совсем другой нумерацией: почти ВСЕ
     * настоящие города стали ошибочно считаться "кастомными".
     */
    private Set<String> getFactoryCityNames() {
        Set<String> names = new HashSet<>();
        File tmp = null;
        try {
            tmp = File.createTempFile("factory_check", ".db", getCacheDir());
            copyDbFromAssets(tmp);
            SQLiteDatabase factoryDb = SQLiteDatabase.openDatabase(tmp.getPath(), null, SQLiteDatabase.OPEN_READONLY);
            Cursor c = factoryDb.rawQuery("SELECT name FROM cities", null);
            while (c.moveToNext()) names.add(c.getString(0));
            c.close();
            factoryDb.close();
        } catch (Exception e) {
            Log.w("RadioName", e.getMessage(), e);
        } finally {
            // Удаляем временный файл В ЛЮБОМ случае — даже если что-то упало
            // на середине, иначе он остаётся мусором в кэше.
    ...
    private void exportUserNames() {
        ...
        try {
            ...
            // Кастомные города — те, которых нет в заводской базе (по имени,
            // не по устаревшему диапазону id).
            Set<String> factoryCities = getFactoryCityNames();
            JSONObject customCities = new JSONObject();
            c = db.rawQuery("SELECT id, name, region FROM cities", null);
            while (c.moveToNext()) {
                String cityName = c.getString(1);
                if (factoryCities.contains(cityName)) continue;
                ...
                customCities.put(cityName, city);
            }
            c.close();
    ...
    private void showCityDialog() {
        ...
        addRow.setBackground(rippleBg(BTN_ACCENT));
        ...
        for (String[] reg : regions) {
            ...
            row.setBackground(rippleBg(BTN_BG));
            row.setGravity(Gravity.CENTER_VERTICAL);
            LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
