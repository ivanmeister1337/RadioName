...
public class MainActivity extends Activity {
    ...
    private void buildUI() {
        ...
        LinearLayout updCard = card(root);
        ...
        LinearLayout diagCard = card(root);
    ...
    private void updateGpsLabel() {
        ...
        // Раньше кнопка обновлялась только при создании UI и навсегда
        // застывала на "Включить"/"Выключить" — теперь обновляем каждый раз.
    ...
    private void toggleGps() {
        ...
        if (autoGpsEnabled) {
            ...
            // Раньше при выключении GPS listener не отписывался и продолжал
            // работать в фоне несмотря на выключенный тумблер — теперь отписываемся.
            unregisterGpsListener(null);
        }
    }
    ...
    private void requestGpsUpdate() {
        try {
            ...
            // Отписываемся от старого listener'а перед регистрацией нового —
            // иначе при каждом вызове (например, из onResume) накапливаются
            // "мёртвые" подписки, которые не дают собрать мусор старым Activity.
            unregisterGpsListener(lm);
            ...
            for (String prov : new String[]{"gps", "passive"}) {
                try {
                    if (lm.isProviderEnabled(prov)) {
                        lm.requestLocationUpdates(prov, 60000, 1000, gpsListener);
                    }
                } catch (Throwable e) {}
            }
        } catch (Exception e) {
    ...
    private void unregisterGpsListener(LocationManager lm) {
        ...
        } catch (Throwable e) { Log.w("RadioName", e.getMessage(), e); }
        gpsListener = null;
    }
