...
public class MainActivity extends Activity {
    ...
    private void toggleGps() {
        ...
        gpsStatusLabel.setText(autoGpsEnabled ? "Вкл — ищу..." : "Выкл");
        gpsStatusLabel.setTextColor(autoGpsEnabled ? 0xFF4CAF50 : 0xFF64B5F6);
        if (autoGpsEnabled) requestGpsUpdate();
    }
    ...
    private void requestGpsUpdate() {
        try {
            ...
            // Сначала попробуем последнее известное местоположение
            Location last = null;
            try { last = lm.getLastKnownLocation(LocationManager.GPS_PROVIDER); } catch (SecurityException e) {}
            if (last == null) try { last = lm.getLastKnownLocation(LocationManager.NETWORK_PROVIDER); } catch (SecurityException e) {}
            if (last != null) {
            ...
            try { lm.requestLocationUpdates(LocationManager.GPS_PROVIDER, 60000, 5000, listener); }
            catch (SecurityException e) {
                requestPermissions(new String[]{android.Manifest.permission.ACCESS_FINE_LOCATION}, 2);
            }
        } catch (Exception e) {
            gpsStatusLabel.setText("Ошибка GPS");
        }
    }
    ...
    private void checkNearestCity(double lat, double lon) {
        ...
        try {
            ...
            while (c.moveToNext()) {
                ...
                if (dist < minDist) {
                    minDist = dist;
                    nearestId = c.getInt(0);
                    nearestName = c.getString(1);
                }
            }
            c.close();

            if (nearestId > 0 && nearestId != currentCityId && minDist < 200) {
                // Переключаем город
                final int newId = nearestId;
                final String newName = nearestName;
                final double dist = minDist;
                runOnUiThread(() -> {
                    currentCityId = newId;
                    currentCityName = newName;
                    db.execSQL("INSERT OR REPLACE INTO settings(key,value) VALUES('current_city_id',?)",
                        new Object[]{String.valueOf(newId)});
                    exportStationsToCache();
                    if (cityLabel != null) cityLabel.setText(newName);
                    if (gpsStatusLabel != null) gpsStatusLabel.setText("✓ " + newName + " (" + (int)dist + " км)");
                    refreshList();
                    Toast.makeText(this, "📍 " + newName + " (" + (int)dist + " км)", Toast.LENGTH_SHORT).show();
                });
            } else if (nearestId > 0) {
                final String name = nearestName;
                final double dist = minDist;
                runOnUiThread(() -> {
                    if (gpsStatusLabel != null)
                        gpsStatusLabel.setText("✓ " + name + " (" + (int)dist + " км)");
                });
            }
        } catch (Exception e) {}
    }
