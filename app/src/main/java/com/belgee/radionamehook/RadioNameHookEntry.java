...
public class RadioNameHookEntry implements IXposedHookLoadPackage {
    ...
    private void adjustPointerTextSize(Object radioRuler, int textLen) {
        try {
            ...
            // ДИАГНОСТИКА: логи показали что pointerPaint (origSize=12.0) НИКОГДА
            // не сбрасывается onDraw() — значит наша правка реально держится, но
            // текст на экране всё равно "большой". Вывод: pointerPaint скорее всего
            // НЕ тот Paint что рисует текст (возможно рисует иглу-указатель — не
            // просто так рядом поля pointerHeight/pointerWidth/pointerColor).
            // Логируем ВСЕ Paint-поля класса разом, чтобы найти реального
            // "текстового" кандидата — сравним размеры разных Paint.
            try {
                for (java.lang.reflect.Field f : radioRuler.getClass().getDeclaredFields()) {
                    if (android.graphics.Paint.class.isAssignableFrom(f.getType())) {
                        f.setAccessible(true);
                        Object val = f.get(radioRuler);
                        if (val instanceof android.graphics.Paint) {
                            android.graphics.Paint p = (android.graphics.Paint) val;
                            XposedBridge.log(TAG + ": PT-allpaints: " + f.getName() +
                                " textSize=" + p.getTextSize() +
                                " isBold=" + p.isFakeBoldText() +
                                " color=" + p.getColor());
                        }
                    }
                }
