...
public class RadioNameHookEntry implements IXposedHookLoadPackage {
                                        ...
                                        new XC_MethodHook() {
                                            ...
                                            protected void afterHookedMethod(MethodHookParam p) {
                                                try {
                                                    ...
                                                    if (display != null) {
                                                        if (display.length() > 24) display = display.substring(0, 23) + "…";
     ...
     */
    /**
     * Меняет размер шрифта ТОЛЬКО у pointerPaint (текст имени станции),
     * не трогая numPaint (цифры шкалы) и scalePaint (чёрточки шкалы).
     * Обнаружено через дамп полей: RadioRulerView содержит три отдельных
     * Paint-объекта — numPaint, scalePaint, pointerPaint.
     */
    ...
    private void adjustPointerTextSize(Object radioRuler, int textLen) {
        try {
            ...
            // Запоминаем оригинальный размер один раз
            if (pointerOriginalTextSize == null) {
                pointerOriginalTextSize = pointerPaint.getTextSize();
            }
            float base = pointerOriginalTextSize;

            float scale;
            if (textLen <= 10) scale = 1.0f;
            else if (textLen <= 14) scale = 0.85f;
            else if (textLen <= 18) scale = 0.72f;
            else if (textLen <= 22) scale = 0.62f;
            else scale = 0.55f;

            pointerPaint.setTextSize(base * scale);
        } catch (Throwable t) {
