...
public class RadioNameHookEntry implements IXposedHookLoadPackage {
                ...
                new XC_MethodHook() {
                    ...
                    protected void afterHookedMethod(MethodHookParam param) {
                        try {
                            ...
                            // Раньше дамп срабатывал ОДИН РАЗ за весь процесс — если
                            // сработал давно, запись могла "уехать" из буфера логов до
                            // того как её проверили. Теперь по требованию: приложение
                            // пишет файл-маркер, хук проверяет его и удаляет после дампа —
                            // можно запускать сколько угодно раз, именно когда нужно.
                            java.io.File dumpMarker = new java.io.File("/sdcard/RadioNames/dump_tree_request.txt");
                            if (dumpMarker.exists()) {
                                try {
                                    XposedBridge.log(TAG + ": === WIDGET TREE DUMP (по запросу) ===");
                                    dumpViewTree(hostView, 0);
                                } catch (Throwable t) {
                                    XposedBridge.log(TAG + ": widget tree dump error: " + t);
                                } finally {
                                    dumpMarker.delete();
                                }
                            }
                                        ...
                                        new XC_MethodHook() {
                                            ...
                                            protected void afterHookedMethod(MethodHookParam p) {
                                                try {
                                                    ...
                                                    if (display != null) {
                                                        if (display.length() > 20) display = display.substring(0, 19) + "…";
    ...
    // (widgetTreeDumped убран — дамп теперь по запросу через файл-маркер, см. выше)

    /** Рекурсивно логирует дерево View — класс, id (hex), и текст если это TextView. */
    private void dumpViewTree(android.view.View v, int depth) {
        try {
            String indent = "";
            for (int i = 0; i < depth; i++) indent += "  ";
            String idHex = "0x" + Integer.toHexString(v.getId());
            String extra = "";
            if (v instanceof android.widget.TextView) {
                CharSequence text = ((android.widget.TextView) v).getText();
                android.graphics.Paint p = ((android.widget.TextView) v).getPaint();
                extra = " text='" + text + "' textSize=" + (p != null ? p.getTextSize() : "?") +
                    " bold=" + (p != null && p.isFakeBoldText());
            }
            XposedBridge.log(TAG + ": TREE " + indent + v.getClass().getSimpleName() +
                " id=" + idHex + extra);
    ...
    private void adjustPointerTextSize(Object radioRuler, int textLen) {
        try {
            ...
            final float FIXED_SCALE = StationsDatabase.getFontScale();
