...
public class RadioNameHookEntry implements IXposedHookLoadPackage {
                                        ...
                                        new XC_MethodHook() {
                                            ...
                                            protected void afterHookedMethod(MethodHookParam p) {
                                                try {
                                                    ...
                                                    if (display != null) {
                                                        // Обрезаем если слишком длинное (>14 символов)
    ...
    private void hookLongPressInRadio(XC_LoadPackage.LoadPackageParam lpparam) {
        try {
            final java.util.Set<Integer> hookedViews = java.util.Collections.newSetFromMap(
                new java.util.WeakHashMap<Integer, Boolean>());
                ...
                new XC_MethodHook() {
                    ...
                    protected void afterHookedMethod(MethodHookParam param) {
                        try {
                            ...
                            if (cs == null || cs.length() < 3 || cs.length() > 30) return;
                            String text = cs.toString();
                            
                            if (text.matches("^[\\d.:/ -]+$")) return;
                            if (text.equals("FM") || text.equals("AM")) return;
                            
                            // Логируем первые 50 кандидатов для диагностики
                            String freq = StationsDatabase.findNameReverse(text);
                            if (logCount[0] < 50) {
                                logCount[0]++;
                                XposedBridge.log(TAG + ": >>> LP setText '" + text + "' reverse=" + freq);
                            }
                            
                            if (freq == null) return;
                            
                            android.widget.TextView tv = (android.widget.TextView) param.thisObject;
                            int viewId = System.identityHashCode(tv);
                            if (hookedViews.contains(viewId)) return;
                            hookedViews.add(viewId);
                            
                            XposedBridge.log(TAG + ": ✓ LP attached to '" + text + "' freq=" + freq);
                            
                            tv.setOnLongClickListener(v -> {
                                try {
                                    String currentText = tv.getText().toString();
                                    String currentFreq = StationsDatabase.findNameReverse(currentText);
                                    if (currentFreq == null) return false;
                                    
                                    android.content.Context ctx = tv.getContext();
                                    while (ctx instanceof android.content.ContextWrapper 
                                           && !(ctx instanceof android.app.Activity)) {
                                    ...
                                    new android.app.AlertDialog.Builder(activity)
                                        .setTitle("FM " + currentFreq)
                                        .setView(input)
                                        .setPositiveButton("Сохранить", (d, w) -> {
                                            String newName = input.getText().toString().trim();
                                            if (!newName.isEmpty()) {
                                                StationsDatabase.updateStation(currentFreq, newName);
                                                tv.setText(newName);
            ...
            XposedBridge.log(TAG + ": ✓ hooked TextView.setText for long-press edit");
        } catch (Throwable t) {
            XposedBridge.log(TAG + ": ✗ longpress hook FAILED: " + t.getClass().getName() + ": " + t.getMessage());
        }
    }
}
