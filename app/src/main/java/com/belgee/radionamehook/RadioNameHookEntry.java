...
public class RadioNameHookEntry implements IXposedHookLoadPackage {
            ...
            XposedHelpers.findAndHookMethod(cls, "getName", new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) {
                    try {
                        ...
                        if (!isCurrentlyTuned) {
                            // Диагностика: этот вызов getName() был про ЧУЖУЮ частоту
                            // (не про то что реально сейчас на экране) — мы его отклонили.
                            // Если это видно в логе часто — подтверждает теорию про
                            // фоновые вызовы (например построение списка избранного).
                            XposedBridge.log(TAG + ": GN-blocked: callFreq=" + normFreq +
                                " actualTuned=" + actualFreq + " (не совпадает, подстановку пропускаем)");
                        }
                        if (isCurrentlyTuned) {
                        ...
                        if (found != null) {
                            ...
                            // Раньше тут ЕЩЁ и принудительно писали в видимый TextView
                            // напрямую (forceUpdateVisibleRadioName) — это оказалось
                            // источником бага "одно и то же имя на всех частотах":
                            // запись шла в ОБЩИЙ TextView при любом вызове getName(),
                            // включая фоновые/несвязанные с текущим экраном. Просто
                            // подмены результата (param.setResult) достаточно — NSMedia
                            // сама вызывает getName() и корректно обновляет свой текст.
                            if (isCurrentlyTuned) {
                                XposedBridge.log(TAG + ": GN-accepted: freq=" + normFreq + " name='" + found + "'");
                            }
                        } else if (isCurrentlyTuned) {
                            // Это точно текущая станция, но данных о ней нет вообще —
                            // подменяем результат явным текстом (тем же безопасным
                            // способом что и обычное имя), а не прямой записью в TextView.
                        ...
                        XposedBridge.hookMethod(m, new XC_MethodHook() {
                            ...
                            protected void afterHookedMethod(MethodHookParam param) {
                                try {
                                    ...
                                    // Та же защита что и в RadioInfo.getName() — объект с которым
                                    // мы работаем может быть чужим/закэшированным (например, от
                                    // последнего выбранного избранного), а не реально текущим.
                                    String normFreq = StationsDatabase.normalizeFreq(freq);
                                    String actualFreq = getActualTunedFreq();
                                    if (actualFreq == null || !actualFreq.equals(normFreq)) return;
    ...
    private void adjustPointerTextSize(Object radioRuler, int textLen) {
        try {
            ...
            boolean wasBoldFake = pointerPaint.isFakeBoldText();
            android.graphics.Typeface tfBefore = pointerPaint.getTypeface();
            suppressPointerBoldHook = true;
            try {
                pointerPaint.setFakeBoldText(false);
                if (tfBefore != null && tfBefore.isBold()) {
                    pointerPaint.setTypeface(android.graphics.Typeface.create(tfBefore, android.graphics.Typeface.NORMAL));
                }
            ...
            // Диагностика: сколько символов, какой масштаб применили, и что
            // РЕАЛЬНО оказалось в Paint после всех манипуляций — покажет
            // сработали ли перехваты вообще, или что-то их перебивает.
            XposedBridge.log(TAG + ": PT >>> textLen=" + textLen + " scale=" + scale +
                " origSize=" + pointerOriginalTextSize +
                " finalSize=" + pointerPaint.getTextSize() +
                " wasBoldFake=" + wasBoldFake + " nowBoldFake=" + pointerPaint.isFakeBoldText() +
                " tfBold=" + (tfBefore != null && tfBefore.isBold()) +
                " nowTfBold=" + (pointerPaint.getTypeface() != null && pointerPaint.getTypeface().isBold()));
        } catch (Throwable t) {
