        ...
        try {
            XposedHelpers.findAndHookMethod(android.appwidget.AppWidgetHostView.class,
                ...
                new XC_MethodHook() {
                    ...
                    // WeakHashMap хранит ПОСЛЕДНЕЕ ПОДСТАВЛЕННОЕ НАМИ значение для
                    // каждого конкретного экземпляра RadioRulerView (не просто флаг
                    // "уже обработан один раз" — раньше было так, и это давало баг:
                    // если launcher САМ сбрасывал pointerValueStr обратно, например
                    // при возврате на главный экран из другого приложения, камеры
                    // заднего вида ИЛИ через меню приложений — мы больше никогда не
                    // перепроверяли этот экземпляр, думая что он "уже обработан".
                    // Теперь на КАЖДОМ обновлении сверяем: если текущее значение
                    // отличается от того что мы сами туда положили — что-то его
                    // сбросило, переприменяем наше сохранённое значение заново.
                    private final java.util.Map<Object, String> lastAppliedDisplay =
                    ...
                    protected void afterHookedMethod(MethodHookParam param) {
                        try {
                                        ...
                                        new XC_MethodHook() {
                                            ...
                                            protected void afterHookedMethod(MethodHookParam p) {
                                                try {
                                                    ...
                                                    // Раньше если имени не было (display == null), мы просто
                                                    // НИЧЕГО не делали — оставляя текст ПРЕДЫДУЩЕЙ станции на
                                                    // экране. Именно так на новой (без имени) частоте виджет
                                                    // застревал на старом названии. Теперь явно показываем
                                                    // саму частоту вместо тишины.
                                                    ...
                                                    // Запоминаем что именно подставили — используется
                                                    // ниже для проверки на сброс на каждом обновлении.
                                                    lastAppliedDisplay.put(p.thisObject, display);
                                                } catch (Throwable t) { }
                                            }
                                        });
                            ...
                            // На КАЖДОМ обновлении виджета (не только для новых
                            // экземпляров) сверяем: совпадает ли текущий pointerValueStr
                            // с тем что мы сами туда положили. Если нет — либо это
                            // совсем новый экземпляр (никогда не подставляли), либо
                            // что-то извне его сбросило — в обоих случаях переприменяем.
                            try {
                                String currentPvs = (String) XposedHelpers.getObjectField(radioRuler, "pointerValueStr");
                                String expected = lastAppliedDisplay.get(radioRuler);

                                if (expected != null) {
                                    // Уже подставляли для этого объекта — если совпадает,
                                    // всё в порядке, ничего не делаем (дешёвый быстрый выход).
                                    if (expected.equals(currentPvs)) return;
                                    // Не совпадает — что-то сбросило, переприменяем НАШЕ
                                    // сохранённое значение напрямую, не пытаясь заново
                                    // разбирать currentPvs (он мог стать чем угодно).
                                    XposedHelpers.setObjectField(radioRuler, "pointerValueStr", expected);
                                    adjustPointerTextSize(radioRuler, expected.length());
                                    radioRuler.invalidate();
                                    return;
                                }

                                // Совсем новый экземпляр — пробуем прочитать pointerValueStr
                                // как частоту (обычно так и есть при первом обнаружении).
                                if (currentPvs != null && !currentPvs.isEmpty()) {
                                    String name = StationsDatabase.findName(currentPvs);
                                    String display = StationsDatabase.formatDisplay(currentPvs, name);
                                    if (display != null) {
                                        final int limit = 19;
                                        if (display.length() > limit) display = display.substring(0, limit - 1) + "…";
                                        ...
                                        radioRuler.invalidate();
                                        lastAppliedDisplay.put(radioRuler, display);
                                    }
                                }
                            } catch (Throwable t) { }
                        } catch (Throwable t) { }
