...
public class RadioNameHookEntry implements IXposedHookLoadPackage {
    ...
    // Ссылка на видимый TextView с именем станции в полноэкранном радио —
    // используется для принудительного обновления текста, если NSMedia
    // не читает подменённое имя через getName() при отрисовке.
    private static volatile java.lang.ref.WeakReference<android.widget.TextView> currentRadioNameView = null;
    // Ссылка на сам фрагмент — нужна чтобы проверить его РЕАЛЬНУЮ текущую
    // частоту (mCurBand/mCurFrequency) и не писать в TextView по любому
    // вызову getName() (их может быть много не связанных с тем что видно
    // на экране — например перебор частот при построении списка станций).
            ...
            XposedHelpers.findAndHookMethod(cls, "getName", new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) {
                    try {
                        ...
                        // Пишем текущую частоту (для кнопки "Переименовать текущую" в
                        // приложении) ТОЛЬКО если это про реально настроенную станцию —
                        // иначе фоновые вызовы getName() (например, при построении списка
                        // станций перебором всех частот) затирают её случайными значениями.
                        //
                        // ВАЖНО: если реальную текущую частоту определить не удалось
                        // (actualFreq == null) — раньше код по умолчанию считал "и так
                        // сойдёт" и всё равно писал. Именно это могло давать редкие случаи
                        // залипания неверного имени. Теперь при неуверенности — НЕ пишем
                        // вообще, лучше ничего не показать чем показать неверное имя.
                        ...
                        if (found != null) {
                            ...
                            // Пишем в видимый TextView ТОЛЬКО если это про реально
                            // настроенную станцию — та самая причина бага "одно и то же
                            // имя на всех частотах" (фоновые вызовы перезаписывали экран).
                            if (isCurrentlyTuned) forceUpdateVisibleRadioName(found);
                        } else if (isCurrentlyTuned) {
                            // Это точно текущая станция, но данных о ней нет вообще —
                            // показываем явно, а не оставляем как есть (например голую
                            // частоту от NSMedia или устаревший текст от предыдущей станции).
                            forceUpdateVisibleRadioName("Нет информации");
                        }
                    } catch (Throwable t) { }
                }
            });
    ...
    /**
     * Принудительно ставит текст в видимый TextView имени станции.
     * Вызывающий код (hookRadioInfoGetName) уже проверил что freq
     * соответствует реально настроенной станции — здесь только сама запись.
     */
    private void forceUpdateVisibleRadioName(final String name) {
        try {
            java.lang.ref.WeakReference<android.widget.TextView> ref = currentRadioNameView;
            if (ref == null) return;
            final android.widget.TextView tv = ref.get();
            if (tv == null) return;
            tv.post(() -> {
                try {
                    if (tv.isAttachedToWindow() && !name.equals(tv.getText().toString())) {
                        tv.setText(name);
                    }
                } catch (Throwable t) { }
            });
                                        ...
                                        new XC_MethodHook() {
                                            ...
                                            protected void afterHookedMethod(MethodHookParam p) {
                                                try {
                                                    ...
                                                    if (display != null) {
                                                        if (display.length() > 40) display = display.substring(0, 39) + "…";
    ...
    private void adjustPointerTextSize(Object radioRuler, int textLen) {
        try {
            Object paintObj = XposedHelpers.getObjectField(radioRuler, "pointerPaint");
            if (!(paintObj instanceof android.graphics.Paint)) return;
            android.graphics.Paint pointerPaint = (android.graphics.Paint) paintObj;

            if (pointerOriginalTextSize == null) pointerOriginalTextSize = pointerPaint.getTextSize();
            trackedPointerPaint = pointerPaint;
            ...
            // и заметно ýже. У нас вся строка (частота+имя) рисуется одним
            // Paint жирным шрифтом целиком — это съедает много лишней ширины.
            suppressPointerBoldHook = true;
            try {
                pointerPaint.setFakeBoldText(false);
                android.graphics.Typeface tf = pointerPaint.getTypeface();
                if (tf != null && tf.isBold()) {
                    pointerPaint.setTypeface(android.graphics.Typeface.create(tf, android.graphics.Typeface.NORMAL));
                }
            ...
            float scale;
            if (textLen <= 10) scale = 1.0f;
            else if (textLen <= 16) scale = 0.85f;
            else if (textLen <= 22) scale = 0.72f;
            else if (textLen <= 30) scale = 0.62f;
            else scale = 0.52f;

            pointerScaleFactor = scale;

            // Форсируем немедленное применение сейчас — на случай если onDraw()
            // не будет вызван сразу после этого. suppress не даёт хуку внизу
            // применить масштаб ДВАЖДЫ к этому же вызову.
            suppressPointerSizeHook = true;
            try {
                pointerPaint.setTextSize(pointerOriginalTextSize * scale);
            } finally {
                ...
                new XC_MethodHook() {
                    ...
                    protected void beforeHookedMethod(MethodHookParam param) {
                        try {
                            ...
                            // Действуем ТОЛЬКО на конкретный объект pointerPaint —
                            // остальные Paint (шкала, цифры, всё остальное в процессе)
                            // не трогаем вообще.
                            if (param.thisObject == trackedPointerPaint && pointerScaleFactor != 1.0f) {
                                float original = (Float) param.args[0];
                                param.args[0] = original * pointerScaleFactor;
                            }
                        } catch (Throwable t) { }
                    }
                });
                ...
                new XC_MethodHook() {
                    ...
                    protected void afterHookedMethod(MethodHookParam param) {
                        try {
                            ...
                            // Запоминаем ссылку — используется хуком RadioInfo.getName()
                            // чтобы принудительно обновлять видимый текст, даже если
                            // NSMedia не читает имя через getName() при отрисовке.
                            currentRadioNameView = new java.lang.ref.WeakReference<>(tv);
