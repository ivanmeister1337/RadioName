        ...
        try {
            XposedHelpers.findAndHookMethod(android.appwidget.AppWidgetHostView.class,
                ...
                new XC_MethodHook() {
                    ...
                    // WeakHashMap хранит ЧАСТОТУ (не готовый текст!) для каждого
                    // конкретного экземпляра RadioRulerView. Раньше хранили готовый
                    // ПОДСТАВЛЕННЫЙ ТЕКСТ и просто сверяли "не сбросило ли его что-то" —
                    // но это не ловило случай когда сама БАЗА поменялась (пользователь
                    // отредактировал имя через долгое нажатие), а pointerValueStr на
                    // экране виджета остался прежним, ведь его никто не трогал. Теперь
                    // храним ЧАСТОТУ и на КАЖДОМ обновлении виджета делаем СВЕЖИЙ поиск
                    // имени — так подхватываются и внешние сбросы (возврат на главный
                    // экран через меню приложений и т.п.), и изменения самой базы.
                    ...
                    protected void afterHookedMethod(MethodHookParam param) {
                        try {
                            ...
                            // Ловит: (а) внешний сброс pointerValueStr (например возврат
                            // на главный экран через меню приложений — оказалось что это
                            // НЕ всегда идёт через setRadioFreq), (б) изменение самой базы
                            // (отредактировали имя через долгое нажатие) без смены частоты.
                            try {
