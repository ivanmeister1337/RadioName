...
public class RadioNameHookEntry implements IXposedHookLoadPackage {
        ...
        try {
            XposedHelpers.findAndHookMethod(android.appwidget.AppWidgetHostView.class,
                ...
                new XC_MethodHook() {
                    ...
                    // Храним ЧАСТОТУ (не готовый текст) для каждого экземпляра
                    // RadioRulerView. Это позволяет на каждом обновлении виджета делать
                    // свежий поиск имени по сохранённой частоте вместо сравнения с ранее
                    // подставленным текстом — так корректно подхватываются и случаи, когда
                    // launcher сам сбрасывает pointerValueStr (например при возврате на
                    // главный экран), и изменения самой базы (переименование станции).
                    private final java.util.Map<Object, String> lastKnownFreq =
                        new java.util.WeakHashMap<>();

                    /** Считает что должно быть на экране для freq, обрезает, применяет. */
                    private String applyDisplay(Object radioRulerObj, String freq) {
                        String name = StationsDatabase.findName(freq);
                        String display = StationsDatabase.formatDisplay(freq, name);
                        if (display == null) display = freq;
                        final int limit = 19;
                    ...
                    protected void afterHookedMethod(MethodHookParam param) {
                        try {
                            ...
                            if (!classHookInstalled) {
                                        ...
                                        new XC_MethodHook() {
                                            ...
                                            protected void afterHookedMethod(MethodHookParam p) {
                                                try {
                                                    ...
                                                    String display = applyDisplay(p.thisObject, freq);
                                                    ...
                                                    // Запоминаем ЧАСТОТУ (не текст) — используется
                                                    // ниже чтобы делать свежий поиск каждый раз.
                                                    lastKnownFreq.put(p.thisObject, freq);
                                                } catch (Throwable t) { }
                                            }
                                        });
                                ...
                                // Возврат на главный экран через меню приложений НЕ вызывает
                                // updateAppWidget (проверено эмпирически — прошлый фикс на
                                // этом хуке не сработал в таком сценарии). Значит нужен
                                // ДРУГОЙ триггер, срабатывающий именно на "окно/view снова
                                // видно" — это штатные Android-колбэки, независимые от
                                // updateAppWidget. Хукаем сразу несколько кандидатов —
                                // какой сработает, тот и подхватит обновление.
                                XC_MethodHook refreshOnVisible = new XC_MethodHook() {
                                    @Override
                                    protected void afterHookedMethod(MethodHookParam p) {
                                        try {
                                            Object view = p.thisObject;
                                            String freq = lastKnownFreq.get(view);
                                            if (freq == null) return;
                                            String currentPvs = (String) XposedHelpers.getObjectField(view, "pointerValueStr");
                                            String freshDisplay = applyDisplay(view, freq);
                                            if (!freshDisplay.equals(currentPvs)) {
                                                XposedHelpers.setObjectField(view, "pointerValueStr", freshDisplay);
                                                adjustPointerTextSize(view, freshDisplay.length());
                                                ((android.view.View) view).invalidate();
                                            }
                                        } catch (Throwable t) { }
                                    }
                                };
                                try {
                                    XposedHelpers.findAndHookMethod(rrClass, "onWindowVisibilityChanged", int.class, refreshOnVisible);
                                    XposedBridge.log(TAG + ": ✓ hooked RadioRulerView.onWindowVisibilityChanged");
                                } catch (Throwable t) {
                                    XposedBridge.log(TAG + ": onWindowVisibilityChanged hook FAIL: " + t);
                                }
                                try {
                                    XposedHelpers.findAndHookMethod(rrClass, "onAttachedToWindow", refreshOnVisible);
                                    XposedBridge.log(TAG + ": ✓ hooked RadioRulerView.onAttachedToWindow");
                                } catch (Throwable t) {
                                    XposedBridge.log(TAG + ": onAttachedToWindow hook FAIL: " + t);
                                }
                            }

                            // На каждом обновлении виджета делаем свежий поиск имени по
                            // запомненной частоте и сверяем с тем что реально на экране.
                            // Ловит и внешний сброс pointerValueStr (не все сценарии
                            // возврата на главный экран идут через setRadioFreq), и
                            // изменение самой базы (переименование станции) без смены частоты.
                            try {
                                String freq = lastKnownFreq.get(radioRuler);
                                String currentPvs = (String) XposedHelpers.getObjectField(radioRuler, "pointerValueStr");

                                if (freq != null) {
                                    String freshDisplay = applyDisplay(radioRuler, freq);
                                    if (!freshDisplay.equals(currentPvs)) {
                                        XposedHelpers.setObjectField(radioRuler, "pointerValueStr", freshDisplay);
                                        adjustPointerTextSize(radioRuler, freshDisplay.length());
                                        radioRuler.invalidate();
                                    }
                                    return;
                                }

                                // Совсем новый экземпляр — ещё не запоминали частоту для
                                // него. Пробуем прочитать pointerValueStr как частоту
                                // (обычно так и есть при первом обнаружении виджета).
                                if (currentPvs != null && !currentPvs.isEmpty()) {
                                    String display = applyDisplay(radioRuler, currentPvs);
                                    if (!display.equals(currentPvs)) {
                                        XposedHelpers.setObjectField(radioRuler, "pointerValueStr", display);
                                        adjustPointerTextSize(radioRuler, display.length());
                                        radioRuler.invalidate();
                                    }
                                    lastKnownFreq.put(radioRuler, currentPvs);
                                }
                            } catch (Throwable t) { }
                        } catch (Throwable t) { }
                    }
    ...
    private void adjustPointerTextSize(Object radioRuler, int textLen) {
        try {
            ...
            // Размер шрифта имени станции зависит от стиля виджета: для
            // "Только имя" — оригинальный (не уменьшенный) размер; для
            // "Частота + имя" — уменьшенный масштаб (StationsDatabase.FONT_SCALE),
            // иначе комбинированная строка не помещается в отведённое место.
            // Совсем длинные имена дополнительно обрезаются с "…" на уровне
            // текста (см. truncateForDisplay), не только за счёт масштаба.
                        ...
                        try {
                            ...
                            tv.setOnLongClickListener(v -> {
                                try {
                                    ...
                                    // Определяем, есть ли реальная информация о станции:
                                    // снимаем цифры/точки/запятые/пробелы/известные единицы
                                    // измерения ("103.4 MHz", "FM 103.4" и т.п.) и смотрим —
                                    // осталось ли что-то ещё после этого.
