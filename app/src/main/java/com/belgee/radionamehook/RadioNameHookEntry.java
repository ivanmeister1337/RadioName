                    ...
                    protected void afterHookedMethod(MethodHookParam param) {
                        ...
                        try {
                            ...
                            if (!radioRulerHooked) {
                                ...
                                // Дамп ВСЕХ полей для поиска bold-флага
                                try {
                                    XposedBridge.log(TAG + ": === RadioRulerView fields dump ===");
                                    for (java.lang.reflect.Field fld : rrClass.getDeclaredFields()) {
                                        fld.setAccessible(true);
                                        Object val = null;
                                        try { val = fld.get(radioRuler); } catch (Throwable t) {}
                                        String tn = fld.getType().getSimpleName();
                                        XposedBridge.log(TAG + ":   " + tn + " " + fld.getName() + " = " + val);
                                    }
                                    // Также поля предка
                                    Class<?> parent = rrClass.getSuperclass();
                                    if (parent != null && !parent.getName().startsWith("android.view")) {
                                        XposedBridge.log(TAG + ": === parent " + parent.getName() + " ===");
                                        for (java.lang.reflect.Field fld : parent.getDeclaredFields()) {
                                            fld.setAccessible(true);
                                            Object val = null;
                                            try { val = fld.get(radioRuler); } catch (Throwable t) {}
                                            XposedBridge.log(TAG + ":   " + fld.getType().getSimpleName() + " " + fld.getName() + " = " + val);
                                        }
                                    }
                                } catch (Throwable t) {
                                    XposedBridge.log(TAG + ": dump error: " + t);
                                }
