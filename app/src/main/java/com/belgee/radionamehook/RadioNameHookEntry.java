...
public class RadioNameHookEntry implements IXposedHookLoadPackage {
                ...
                new XC_MethodHook() {
                    ...
                    protected void afterHookedMethod(MethodHookParam param) {
                        try {
                            ...
                            // ПОИСК: возможно "FM 87.5" — отдельный TextView-сосед
                            // RadioRulerView внутри виджета, а не часть pointerValueStr.
                            // Проходим ВСЁ дерево виджета один раз и логируем каждый
                            // View — его класс, id и текст (если TextView).
                            if (!widgetTreeDumped) {
                                widgetTreeDumped = true;
                                try {
                                    XposedBridge.log(TAG + ": === WIDGET TREE DUMP ===");
    ...
    private static volatile boolean widgetTreeDumped = false;
    ...
    private void adjustPointerTextSize(Object radioRuler, int textLen) {
        try {
            ...
            final float FIXED_SCALE = 0.56f; // 64.0 * 0.56 ≈ 35.8 (было 0.42 ≈ 26.9 — слишком мелко)
