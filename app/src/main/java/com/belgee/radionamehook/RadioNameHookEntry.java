...
public class RadioNameHookEntry implements IXposedHookLoadPackage {
            ...
            XposedHelpers.findAndHookMethod(cls, "getName", new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) {
                    try {
                        ...
                        if (found != null) {
                            // Раньше здесь НЕ БЫЛО обрезки вообще — только у виджета на
                            // главном экране. В полноэкранном режиме места больше, но
                            // не бесконечно — режем менее агрессивно чем у виджета.
                            if (found.length() > 28) found = found.substring(0, 27) + "…";
                            param.setResult(found);
                            // На случай если NSMedia не перечитывает getName() при отрисовке —
                            // принудительно обновляем видимый TextView напрямую.
                            forceUpdateVisibleRadioName(found);
                        }
                    } catch (Throwable t) { }
                }
            });
    ...
    /** Принудительно ставит текст в видимый TextView имени станции, если он ещё на экране. */
     ...
     * ВАЖНО (честно): это ГИПОТЕЗА, не подтверждённый факт. Поле pointerHeight
     * из дампа полей (20.0 при DEFAULT_POINTER_HEIGHT=42.0, рядом
     * DEFAULT_POINTER_WIDTH=4.0) больше похоже на геометрию иглы-указателя
     * (тонкая линия/треугольник), а НЕ на размер шрифта — трогать его не стал,
     * чтобы не сломать визуал иглы вместо текста.
     * Единственная НАДЁЖНАЯ защита от переполнения виджета — обрезка текста
     * в самом display (см. вызовы ниже, лимит символов), она гарантированно
     * работает независимо от того, сработает ли это уменьшение шрифта.
     */
    ...
    private void adjustPointerTextSize(Object radioRuler, int textLen) {
        try {
            ...
            else if (textLen <= 14) scale = 0.85f;
            else if (textLen <= 18) scale = 0.72f;
            else scale = 0.62f;
