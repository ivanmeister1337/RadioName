...
public class StationsDatabase {
    ...
    // Масштаб шрифта для стиля "Частота + имя" — подобран и зафиксирован.
    // Раньше настраивался через файл и пресеты в приложении (для подбора
    // на реальном устройстве без пересборки APK) — значение найдено, подбор
    // больше не нужен, простая константа.
    private static final float FONT_SCALE = 0.89f;
    ...
    /** Текущий стиль виджета (0="Только имя", 1="Частота + имя") — для раздельной настройки шрифта. */
    public static int getDisplayStyle() {
        if (System.currentTimeMillis() - lastStyleLoad > RELOAD_INTERVAL_MS) loadStyle();
        return displayStyle;
    }

    /** Масштаб шрифта для стиля "Частота + имя" — зафиксированное значение. */
